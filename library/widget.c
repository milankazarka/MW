...
int milanWidget::addText( milanText *text )
{
  ...
  objects = wobject;
  wobject->type = _WIDGET_TYPE_TEXT;
  wobject->data = text;
  return 0;
}
...
int milanWidget::drawOnWindow( milanWindow *window )
{
  ...
  while(current)
  {
    ...
    else if (current->type == _WIDGET_TYPE_TEXT)
    {
      milanText *mt = (milanText*)current->data;
      gl->drawText(mx+20,my+20,(unsigned char*)mt->myStr);
    }
  ...
  while(current)
  {
    if (current->type == _WIDGET_TYPE_TEXT)
    {
      milanText *mt = (milanText*)current->data;
      gl->drawText(mx+20,my+20,(unsigned char*)mt->myStr);
    }
...
int milanWidget::drawOnWidget( milanWidget *widget )
{
  printf("milanWidget::drawOnWidget\n");
  if (!widget)
  {
    mprintf("milanWidget::drawOnWidget error, invalid widget\n");
    return 1;
  }
  return 0;
}
